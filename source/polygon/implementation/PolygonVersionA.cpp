...
//! Constructor.
PolygonVersionA::PolygonVersionA()
{
    mMaskBuffer = NULL;
    mWindingBuffer = NULL;
 ...
 *  be cropped.
 */
bool PolygonVersionA::init(unsigned int aWidth, unsigned int aHeight)
{
 ...
 */
void PolygonVersionA::renderEvenOdd(BitmapData *aTarget, const Polygon *aPolygon, unsigned long aColor, const MATRIX2D &aTransformation)
{
 ...
 *  \param aTransformation the transformation matrix.
 */
void PolygonVersionA::renderNonZeroWinding(BitmapData *aTarget, const Polygon *aPolygon, unsigned long aColor, const MATRIX2D &aTransformation)
{
    if (!isSupported(aTarget->mFormat))
        return;
...
void PolygonVersionA::setClipRect(unsigned int aX, unsigned int aY, unsigned int aWidth, unsigned int aHeight)
{
    unsigned int x = aX;
    unsigned int y = aY;
...
void PolygonVersionA::addEvenOddEdge(const PolygonEdge &aEdge)
{
    static const RATIONAL offsets[SUBPIXEL_COUNT] = SUBPIXEL_OFFSETS;

    int y;
    RATIONAL x = aEdge.mX;
    RATIONAL slope = aEdge.mSlope;
    for (y = aEdge.mFirstLine; y <= aEdge.mLastLine; y++)
    {
        int ySub = y & (SUBPIXEL_COUNT - 1);
        int xp = (int)(x + offsets[ySub]);
...
void PolygonVersionA::fillEvenOdd(BitmapData *aTarget, unsigned long aColor, const ClipRectangle &aClipRect)
{
    unsigned int x,y;
...
void PolygonVersionA::addNonZeroEdge(const PolygonEdge &aEdge)
{
    static const RATIONAL offsets[SUBPIXEL_COUNT] = SUBPIXEL_OFFSETS;
...
void PolygonVersionA::fillNonZero(BitmapData *aTarget, unsigned long aColor, const ClipRectangle &aClipRect)
{
    unsigned int x,y;
...
bool PolygonVersionA::isSupported(BitmapData::BitmapFormat aFormat)
{
